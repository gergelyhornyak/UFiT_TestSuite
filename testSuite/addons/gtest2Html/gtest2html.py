...
def generate_total_test_summary(xml_testsuites_node):
    ...
    test_project_name = get_xml_attribute(str, xml_testsuites_node, 'project', 'ufit-test-suite')
...
def generate_single_testcase_rows(xml_testsuite_node):
    ...
    suite_name = get_xml_attribute(str, xml_testsuite_node, 'name', '-undefined-')
    if len(xml_testcase_nodes) == 0:
        print("Warning: No nodes 'testcase' found in testsuite element with name {!r}.".format(suite_name))
    ...
    for idx, xml_testcase_node in enumerate(xml_testcase_nodes):
        ...
        property_list = []
        xml_properties = xml_testcase_node.findall('./properties/property')
        for prop in xml_properties:
            p_name = get_xml_attribute(str, prop, 'name', '')
            p_value = get_xml_attribute(str, prop, 'value', '')
            if p_name:
                property_list.append("{}: {}".format(p_name, p_value))
        
        # Join tags with a comma or space for display in HTML
        test_tags = ", ".join(property_list)
        ...
        check_for_unkown_attributes(
            xml_testcase_node, ['name', 'time', 'status', 'classname'])
...
def generate_single_test_result_listings(xml_testsuites_node):
    ...
    for xml_testsuite_node in xml_testsuite_nodes:
        ...
        testsuite_tags = get_xml_attribute(str, xml_testsuite_node, 'property', '')
        ...
        check_for_unkown_attributes(xml_testsuite_node, ['name', 'tests',
                                                         'failures',  'disabled', 'time', 'property'])
