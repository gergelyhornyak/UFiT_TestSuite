...
extern "C" {
    ...
    void __ufit_functions_fortran_MOD_intercept_boundary_c100(double* pos_in, double* direction, double* delta_out);
}

TEST(InterceptBoundaryC100, EqualsOne) {
    ...
    __ufit_functions_fortran_MOD_intercept_boundary_c100(position, direction, &delta_out);

    EXPECT_DOUBLE_EQ(delta_out,expected_delta_out) << "intercept_boundary_c100 has bug \n";
}

TEST(InterceptBoundaryC100, BoundaryCaseLessThan) {
    ...
    EXPECT_LT(delta_out,expected_delta_out) << "intercept_boundary_c100 has bug \n";
    // EXPECT_EQ(delta_out,expected_delta_out) << "intercept_boundary_c100 has bug \n";
    
}

TEST(InterceptBoundaryC100, HandlesZeroDirectionUsingEpsilon) {
    RecordProperty("priority", "high");
    __ufit_definitions_fortran_MOD_grid1min = 0.0;
    __ufit_definitions_fortran_MOD_grid1max = 10.0;
    __ufit_definitions_fortran_MOD_grid2min = 0.0;
    ...
    __ufit_functions_fortran_MOD_intercept_boundary_c100(position, direction, &delta_out);
    // delta_out will be 0.5, but expected is 1.0
    EXPECT_DOUBLE_EQ(delta_out,expected_delta_out) << "The function failed to handle a zero-direction component safely \n";
